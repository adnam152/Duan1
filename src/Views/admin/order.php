...
<table class="table" id="order_table">
    ...
    <tbody>
        <?php foreach ($allBills as $index => $bill) : ?>
            <tr data-accordion="<?= $index ?>" class="pointer text-center <?= $bill['status'] == 0 ? 'table-info' : 'bg-success' ?>">
                <td><?= $bill['id'] ?></td>
                <td><?= $bill['username'] ?? 'Khách hàng chưa đăng nhập' ?></td>
                <td><?= $bill['create_at'] ?></td>
                <td><?= number_format($bill['total_price']) ?>đ</td>
                <td data-type="status"><?= $bill['status'] == 1 ? 
        ...
        ajaxRequest('/api/order?updatestatus=' + billId, 'GET')
            .then(res => {
                if (res == "success") {
                    ...
                    btn.disabled = true;
                    // Đang xử lý -> đã giao hàng
                    const status = btn.closest('tr').querySelector('td[data-type="status"]');
                    status.innerHTML = '<span class="badge bg-success">Đã giao hàng</span>';
                } else alert("Có lỗi xảy ra");
            })
    }
</script>