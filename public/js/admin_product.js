...
(()=>{
    ...
    addBtn.onclick = function () { // add btn
        resetBtn.click();
        add.hidden = false;
        update.hidden = true;
    ...
    updateBtns.forEach(btn => { // update btn
    ...
    deleteBtn.forEach(btn =>{ // delete btn
        btn.onclick = function(){
    ...
    deleteDetailBtn.forEach(btn =>{ // delete detail btn
        btn.onclick = function(){
            const productId = btn.closest('tr').dataset.productId;
...
// Add Image
(()=>{
    const inputImages = document.querySelectorAll('input[name="add_img_btn[]"]');
    inputImages.forEach(inputImage => {
        inputImage.onchange = function () {
            const submitBtn = inputImage.closest('form').querySelector('input[type="submit"]');
            submitBtn.click();
        }
    })
...
// Delete Image
(()=>{
    const deleteImages = document.querySelectorAll('button[name="delete_img_btn"]');
    deleteImages.forEach(btn =>{
        btn.onclick = function(){
            if(confirm("Xác nhận xóa?")){
                location.href = location.href + '?delete_img=&img_id=' + btn.dataset.imageId;
            }
        }
    })
})();